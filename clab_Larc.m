...
if ~use_uplab
%     % Use obsevered L values only (could interpolate between these)
...
%     [h0,h1,c] = arcparams_cielab(L, rgbgamut);
    ...
    % Look up chroma
    c = min(rgbgamut.lchmesh.cgrid(hli, rgbgamut.lchmesh.Lvec==L));
...
%     % Use automated definitions based on observed search spaces
%     [h0,h1,c] = arcparams_uplab(L, rgbgamut);
...
% =========================================================================
function [h0,h1,c] = arcparams_uplab(L, rgbgamut)
    % ---------------------------------------------------------------------
    if nargin<2
